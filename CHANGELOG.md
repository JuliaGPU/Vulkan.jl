...
- ![BREAKING][badge-breaking] The wrapped Vulkan specification was updated from 1.3.240 to 1.4.321, along with [VulkanCore.jl](https://github.com/JuliaGPU/VulkanCore.jl). New types, functions and extensions are available. Breaking in one respect: 41 intermediate `_`-prefixed names for extensions that 1.4 promoted to core are gone, among them `_PhysicalDeviceLineRasterizationFeaturesEXT`, `_QueueFamilyGlobalPriorityPropertiesKHR` and `_cmd_push_descriptor_set_khr`. Their high-level equivalents are unaffected.
- ![Bugfix][badge-bugfix] Structs that Clang emits as an opaque blob -- those containing a union or a bitfield -- are constructible again. Such a struct gets one explicitly typed constructor and Julia does not convert arguments to make a method match, so passing the high-level `Format`, `IndexType` or handle that the wrapper's own signature asks for was a `MethodError`. The wrapper now converts each member to its raw type at the construction site, which reaches all twenty-three of the blob structs it constructs -- among them every acceleration-structure geometry and build info, the micromap and cluster builds, `VkDescriptorGetInfoEXT` and `VkRenderingAttachmentInfo`.
- ![Bugfix][badge-bugfix] `setproperties` is defined for opaque structs, which 46 APIs returning one needed: `ConstructionBase` refuses a type whose `propertynames` is overloaded, and `_initialize_core` sets `sType`/`pNext` through exactly that call, so those wrappers threw before ever reaching the driver. Found through `get_pipeline_executable_statistics_khr`, which does work and reports twenty-six statistics per compute pipeline on RADV.
- ![Bugfix][badge-bugfix] A flag member can be left at its default again. Optional members default to the integer `0`, and the wrapper turned a flag into its raw `Vk*FlagBits` by reading `.val`, which an integer does not have -- so `_RenderingAttachmentInfo` could not be constructed without naming `resolve_mode`, and the same held for the nineteen flag members with such a default. A `flag_value` taking the integer and the `BitMask` alike replaces it, by dispatch rather than a width cast so that a 64-bit flag cannot be silently truncated.
- ![Bugfix][badge-bugfix] A length member counting a FIXED-SIZE array is kept in the high-level struct rather than dropped. The count is dropped for a pointer array because the `Vector` that replaces it carries its own length; an `NTuple` does not. The 1.4 registry annotates `memoryTypes` with `len="memoryTypeCount"`, which without this would have made `PhysicalDeviceMemoryProperties` report all 32 memory types on a device that has 11, the other 21 zeroed and indistinguishable, with no way left to ask. Also affects `PhysicalDeviceGroupProperties` and `QueueFamilyGlobalPriorityProperties`.
- ![Bugfix][badge-bugfix] The three unions whose alternatives are pointers -- `VkIndirectCommandsTokenDataEXT`, `VkIndirectExecutionSetInfoEXT` and `VkClusterAccelerationStructureOpInputNV`, all new in 1.4 -- take a pointer rather than a value. Their 18 constructors previously promised a wrapper struct and handed it to a raw constructor that stores a pointer. As with `VkDescriptorDataEXT`, the caller owns the pointee's lifetime.
- ![Enhancement][badge-enhancement] Loading no longer requires a Vulkan loader to be present. `__init__` filled the dispatch table unconditionally, which `dlopen`s libvulkan and so made the package impossible to even precompile on a machine without a driver. Without a loader the package now compiles nothing at all -- `Vulkan.HAS_LOADER` is `false`, and the bindings, dispatch table and export list are not built -- so depending on it from a package that only uses Vulkan on some platforms is free on the others. The answer is fixed at precompile time, so installing a driver afterwards needs `Pkg.precompile(; force = true)`.
