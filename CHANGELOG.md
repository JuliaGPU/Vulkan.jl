...
- ![BREAKING][badge-breaking] The wrapped Vulkan specification was updated from 1.3.240 to 1.4.321, along with [VulkanCore.jl](https://github.com/JuliaGPU/VulkanCore.jl). `PhysicalDeviceMemoryProperties` no longer has `memory_type_count`/`memory_heap_count`, because 1.4 removed them: `memory_types` and `memory_heaps` are truncated to the real counts instead, so use `length` on those.
