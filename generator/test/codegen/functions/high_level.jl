@testset "High-level functions" begin
    test_ex(VulkanGen.promote_hl(APIFunction(api.functions[:vkEnumeratePhysicalDevices], false)), :(
    ...
    test_ex(VulkanGen.promote_hl(APIFunction(api.functions[:vkGetPerformanceParameterINTEL], false)), :(
    ...
    test_ex(VulkanGen.promote_hl(APIFunction(api.functions[:vkDestroyInstance], false)), :(
        function destroy_instance(instance; allocator = C_NULL)
            _destroy_instance(instance; allocator)
        end
    ))

    test_ex(VulkanGen.promote_hl(APIFunction(api.functions[:vkGetPhysicalDeviceProperties2], false)), :(
    ...
    test_ex(VulkanGen.promote_hl(APIFunction(api.functions[:vkGetPhysicalDeviceImageFormatProperties2], false)), :(
    ...
    test_ex(VulkanGen.promote_hl(APIFunction(api.constructors[:vkCreateInstance], false)), :(
