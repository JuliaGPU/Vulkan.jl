...
function SwapchainSetup(device::DeviceSetup, surface::SurfaceKHR; format, buffering=3, colorspace=COLOR_SPACE_SRGB_NONLINEAR_KHR, layers=1, usage=IMAGE_USAGE_COLOR_ATTACHMENT_BIT, sharing_mode=SHARING_MODE_EXCLUSIVE, present_mode=PRESENT_MODE_IMMEDIATE_KHR, clipped=false, composite_alpha_mode=COMPOSITE_ALPHA_OPAQUE_BIT_KHR, old_swapchain=C_NULL)
    ...
    swapchain = SwapchainKHR(device, SwapchainCreateInfoKHR(surface, buffering, format, colorspace, capabilities.current_extent, layers, UInt32(usage), sharing_mode, Int[], capabilities.current_transform, composite_alpha_mode, present_mode, clipped; old_swapchain))
