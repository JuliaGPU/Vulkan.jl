...
# `get_pipeline_executable_statistics_khr` is one caller that depends on this:
# `VkPipelineExecutableStatisticKHR` is such a blob, `_initialize_core` patches
# `sType`/`pNext` into it with `setproperties`, and without a method here that
# call fails before returning anything — taking a driver's register and spill
# counts with it, and any check built on them (`Lava.checked_workgroup`).
